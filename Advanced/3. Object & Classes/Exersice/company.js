class Company {
    ...
    bestDepartment() {
        let sortedDepartment = [...this.departments].sort(
            ([aName, aWorkers], [bName, bWorkers]) => {
                return (
                    this._getAverageSalary(bName) -
                    this._getAverageSalary(aName)
                );
            }
        );
        let [bestDepartmentName, bestDepartmentWorkers] = sortedDepartment[0];
        bestDepartmentWorkers.sort((a, b) => a.compareTo(b));
        let bestDepartmentString = `Best Department is: ${bestDepartmentName}\nAverage salary: ${this._getAverageSalary(
            bestDepartmentName
        ).toFixed(2)}`;
        let workersString = bestDepartmentWorkers
            .map((x) => x.toString())
            .join("\n");
        return `${bestDepartmentString}\n${workersString}`;
    }
    _getAverageSalary(departmentName) {
        let departmentWorker = this.departments.get(departmentName);

        let averageSalary =
            departmentWorker.reduce((acc, w) => acc + w.salary, 0) /
            departmentWorker.length;
