class VeterinaryClinic {
    ...
    newCustomer(ownerName, petName, kind, procedures) {
        ...
        if (currentOwner && currentPet) {
            ...
            if (pet.procedures.length > 0) {
                throw new Error(`This pet is already registered under ${currentOwner.ownerName} name! ${currentPet.petName} is on
                 our lists, waiting for ${currentPet.procedures.join(', ')}.`)
    ...
    toString() {
        let busyPercantage = Math.floor(this.currentWorkload / this.capacity * 100);
        let result = `${this.clinicName} is ${busyPercantage}% busy today!`;
        result += `\n`
